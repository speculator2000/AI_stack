...
# --- 2. Core Market Depth Simulation Engine ---
class MarketDepthAnalyzer:
    ...
    def calculate_order_book_analytics(self):
        ...
        analytics = {}
        for key, snapshot in list(self.depth_snapshots.items())[:10]:
            bids = snapshot['bids']
            asks = snapshot['asks']
...
with st.sidebar:
    ...
    if not HAS_DEPTHSIM:
        st.info("ℹ️ Using built-in simulation. For enhanced features, install: `pip install conflux-depthsim`")
    ...
    with col1:
        ...
        if st.button("Calculate Impact"):
            ...
            if impact:
                ...
                st.error("Failed to calculate impact. Insufficient liquidity.")
